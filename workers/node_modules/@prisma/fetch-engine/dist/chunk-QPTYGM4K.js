...
var chunk_QPTYGM4K_exports = {};
__export(chunk_QPTYGM4K_exports, {
  download: () => download,
...
module.exports = __toCommonJS(chunk_QPTYGM4K_exports);
...
var import_chunk_B7Q6Z2SD = require("./chunk-B7Q6Z2SD.js");
...
var require_stream = (0, import_chunk_AH6QHEOA.__commonJS)({
  "../../node_modules/.pnpm/execa@5.1.1/node_modules/execa/lib/stream.js"(exports, module2) {
    "use strict";
    var isStream = (0, import_chunk_B7Q6Z2SD.require_is_stream)();
...
var import_fs_extra = (0, import_chunk_AH6QHEOA.__toESM)((0, import_chunk_5T2KHPZI.require_lib)());
var import_p_filter = (0, import_chunk_AH6QHEOA.__toESM)(require_p_filter());
var import_temp_dir = (0, import_chunk_AH6QHEOA.__toESM)((0, import_chunk_B7Q6Z2SD.require_temp_dir)());
...
async function download(options) {
  ...
  if (binariesToDownload.length > 0) {
    const cleanupPromise = (0, import_chunk_RIFXCDQ6.cleanupCache)();
    let finishBar;
    let setProgress;
    ...
    const promises = binariesToDownload.map((job) => {
      const downloadUrl = (0, import_chunk_5T2KHPZI.getDownloadUrl)({
        channel: "all_commits",
...
async function binaryNeedsToBeDownloaded(job, nativePlatform, version) {
  ...
  if (cachedFile) {
    if (job.skipCacheIntegrityCheck === true) {
      await (0, import_chunk_5T2KHPZI.overwriteFile)(cachedFile, job.targetFilePath);
      return false;
    }
...
async function getCachedBinaryPath({
  ...
  const cacheDir = await (0, import_chunk_5T2KHPZI.getCacheDir)(channel, version, binaryTarget2);
  if (!cacheDir) {
    return null;
  }
...
async function downloadBinary(options2) {
  ...
  const { sha256, zippedSha256 } = await (0, import_chunk_B7Q6Z2SD.downloadZip)(downloadUrl, targetFilePath, progressCb);
  if (progressCb) {
    progressCb(1);
  }
...
async function saveFileToCache(job, version, sha256, zippedSha256) {
  const cacheDir = await (0, import_chunk_5T2KHPZI.getCacheDir)(channel, version, job.binaryTarget);
  if (!cacheDir) {
    return;
  }
  ...
  try {
    await (0, import_chunk_5T2KHPZI.overwriteFile)(job.targetFilePath, cachedTargetPath);
    if (sha256 != null) {
